...
import (
	...
	"github.com/Blackmocca/utils/orm"
...
type Order struct {
	...
	ChefID    *uuid.UUID             `json:"-" db:"chef_id" type:"uuid"`
...
func TestMapper(t *testing.T) {
	...
	var runQuery = func(t *testing.T, client *Client, sql string, options ...orm.MapperOption) []*Order {
	...
	var runQueryContext = func(t *testing.T, ctx context.Context, client *Client, sql string, options ...orm.MapperOption) ([]*Order, error) {
		rows, err := client.GetClient().Queryx(sql)
		if err != nil {
			t.Error(err)
		}
		...
		mapper, err := orm.OrmContext(ctx, new(Order), rows, options[0])
		if err != nil {
			return nil, err
		}
		return mapper.GetData().([]*Order), err
	}
	...
	t.Run("success_cancel_with_order_left_join_both_topping_batters_and_join_chef", func(t *testing.T) {
		orders := getData(t, "./testdata/orders.json")
		db, dbmock, err := sqlmock.New()
		if err != nil {
			t.Fatalf("an error '%s' was not expected when opening a stub database connection", err)
		...
		_, merr := runQueryContext(t, ctx, client, sql)
		assert.Equal(t, context.Canceled.Error(), merr.Error())
	})
