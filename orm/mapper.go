...
func orm(ctx context.Context, model interface{}, rows *sqlx.Rows, options MapperOption) (Mapper, error) {
	...
	for rows.Next() {
		...
		group.SetLimit(options.grouplimit)
	...
	if len(mapper.modelStructs) > 1 && options.autobinding {
		...
		var bind = func(ctx context.Context, group *errgroup.Group, elem reflect.Value, refFields []string, allmodels []modelStruct, options MapperOption) {
			group.Go(func() error {
				defer func() {
					if panicErr := recovery(); panicErr != nil {
						err = panicErr
				...
				return bindReference(ctx, elem, refFields, allmodels, options)
			})
		}
		/* orm sub component */
		if refIndexes := modelStructs(mapper.modelStructs).GetListReferenceModelIndex(); len(refIndexes) > 0 && len(options.pkFields) > 0 {
			...
			refGroup.SetLimit(options.grouplimit)
				...
				func(ctx context.Context, refModel modelStruct, refIndex int) {
					refGroup.Go(func() error {
						...
						for index := 0; index < subMainModel.modelSlice.Len(); index++ {
							bind(subGroupCtx, group, subMainModel.modelSlice.Index(index), subMainModel.refFields, refModel.subRefModel, mapper.options)
						}
						if err := group.Wait(); err != nil {
							return err
						}
		...
		if mainModel.modelSlice.Len() > 0 && len(mainModel.refFields) > 0 {
			...
			for index := 0; index < mainModel.modelSlice.Len(); index++ {
				bind(ctx, group, mainModel.modelSlice.Index(index), mainModel.refFields, mapper.modelStructs, mapper.options)
			}
			if err := group.Wait(); err != nil {
				return mapper, err
			}
	...
	if len(fields) > 0 {
		for _, field := range fields {
			if field.Name() != TABLE_FIELD_NAME {
				tagVal := field.Tag(TAGNAME)
				columename := strings.Split(tagVal, ",")[0]

				if columename != "" && columename != "-" && !existsColumnOption(tagVal, noSelector) {
...
func bindReference(ctx context.Context, mainElem reflect.Value, mainRefFieldNames []string, allModels []modelStruct, options MapperOption) error {
	...
	if len(mainRefFieldNames) > 0 {
		...
		for _, refField := range mainRefFieldNames {
			...
			func(mainElem reflect.Value, refField string, pkFieldRefDataField reflect.Value, allModels []modelStruct) {
			...
			}(mainElem, refField, pkFieldRefDataField, allModels)
		}
		if err := group.Wait(); err != nil {
			return err
		}
...
func isJoin(mainFaith *structs.Struct, refModel interface{}, fkCol1Keys []string, fkCol2Keys []string) bool {
	checkEqual := func(fkCol1, fkCol2 string) bool {
		...
		linkID := structs.New(refModel).Field(fkCol2).Value()
