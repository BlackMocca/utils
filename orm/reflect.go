...
const (
	noSelector columnOption = "nselector"
)

func existsColumnOption(tagvalue string, option columnOption) bool {
	splits := strings.Split(tagvalue, ",")
	if len(splits) > 1 {
		for _, item := range splits {
			if strings.EqualFold(item, string(option)) {
				return true
			}
		}
	}
...
func getTagValue(faith *structs.Struct, field string, tag string) string {
	...
	if f, ok := faith.FieldOk(field); ok {
		var tagStr = f.Tag(tag)

		// if db tag split for got column value
		if tag == TAGNAME {
			tagStr = strings.Split(tagStr, ",")[0]
		}
		tagVal = strings.TrimSpace(tagStr)
	}
...
func getStructFields(faith *structs.Struct) *sync.Map {
	...
	for _, f := range fields {
		tagCol := strings.Split(f.Tag(TAGNAME), ",")[0]
		if tagCol != "" && tagCol != "-" {
...
func setFieldFromType(field *structs.Field, data interface{}) error {
	var tag = strings.Split(field.Tag(TAG_TYPE), ",")[0]
	if tag != "" && tag != "-" {
